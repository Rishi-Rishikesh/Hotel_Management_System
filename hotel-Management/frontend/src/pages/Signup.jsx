    ...
    try {
      ...
      while (retries >= 0) {
        try {
          ...
          throw firebaseError; // Re-throw if not a retryable error
        }
      }

      // Extract UID from the created Firebase user
      const firebaseUid = firebaseUser.user.uid;

      // Register user in MongoDB, sending Firebase UID
      const response = await axios.post(
        "http://localhost:4000/api/guests/signup",
        { email, password, firebaseUid }, // Include firebaseUid here
        {
      ...
      console.error("Signup error:", error, "Error code:", error.code, "Error message:", error.message);
      ...
      if (error.code) {
        switch (error.code) {
            ...
            message = (
              <>
                ...
                <ul className="list-disc ml-5 mt-2 text-xs">
                  ...
                  <li>Firewall/antivirus is not blocking Firebase</li>
                  <li>VPN is not interfering with the connection</li>
                </ul>
              </>
            );
            break;
          ...
          case "auth/firebase-app-check-token-is-invalid":
            message = "App Check verification failed. Please try again.";
            break;
      ...
      setErrorMessage(message);
      toast.error(typeof message === "string" ? message : "Signup failed due to network issues.");
    } finally {
